...
async def call_openai_async(session, messages, model=DEFAULT_MODEL):
    """
    Asynchronously call the OpenAI Chat Completion API with the provided messages.
        ...
        async with session.post(OPENAI_URL, headers=headers, json=payload) as resp:
            if resp.status == 200:
                ...
                try:
                    ...
                    print("Unexpected OpenAI response structure:", result)
                    return None
            else:
